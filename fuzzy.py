...
st.sidebar.markdown("---")
st.sidebar.info("Aplikasi Integrasi: LPI Generator & Volume Coating/Welding Calculator.")
...
if main_menu == "LIQUID PENETRANT REPORT":
    ...
    if uploaded_logo is not None:
        # Menampilkan preview logo yang diunggah di tengah/kiri atas
    ...
    auto_form_no = f"05/LPI/DAMAC/{month_year_slug}" [cite: 1]
    ...
    with col1:
        ...
        equipment = st.text_input("Equipment / System", value="FUEL PIPE") [cite: 1] 

    with col2:
        st.text_input("Form No (Otomatis)", value=auto_form_no, disabled=True)
        st.text_input("Date (Otomatis)", value=date_str, disabled=True)
        drawing_no = st.text_input("Drawing No", value="ISO-JKT01-003") [cite: 1] 
    ...
    if st.button("🚀 Generate Final Report Layout"):
        ...
        # JIKA LOGO SUDAH DIUNGGAH, ATUR POSISI LOGO DI ATAS KOP LAPORAN
        if uploaded_logo is not None:
